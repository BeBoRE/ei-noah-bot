import { pgTable, serial, varchar, timestamp, foreignKey, integer, unique, index, boolean } from "drizzle-orm/pg-core"
...
export const customRole = pgTable("custom_role", {
	id: varchar("id", { length: 255 }).primaryKey().notNull(),
	ownerId: integer("owner_id").notNull().references(() => guildUser.id, { onUpdate: "cascade" } ),
	...
	guildId: varchar("guild_id", { length: 255 }).notNull().references(() => guild.id, { onUpdate: "cascade" } ),
});
...
export const guildUser = pgTable("guild_user", {
	guildId: varchar("guild_id", { length: 255 }).notNull().references(() => guild.id, { onUpdate: "cascade" } ),
...
(table) => {
	return {
		guildUserGuildIdUserIdUnique: unique("guild_user_guild_id_user_id_unique").on(table.guildId, table.userId),
	}
});

export const lobbyNameChange = pgTable("lobby_name_change", {
	id: serial("id").primaryKey().notNull(),
	guildUserId: integer("guild_user_id").notNull().references(() => guildUser.id, { onUpdate: "cascade" } ),
...
export const quote = pgTable("quote", {
	...
	creatorId: integer("creator_id").notNull().references(() => guildUser.id, { onUpdate: "cascade" } ),
	date: timestamp("date", { withTimezone: true, mode: 'string' }),
});

export const tempChannel = pgTable("temp_channel", {
...
(table) => {
	return {
		tempChannelGuildUserIdUnique: unique("temp_channel_guild_user_id_unique").on(table.guildUserId),
	}
});
...
export const guild = pgTable("guild", {
	...
	seperateTextChannel: boolean("seperate_text_channel").default(false).notNull(),
});
