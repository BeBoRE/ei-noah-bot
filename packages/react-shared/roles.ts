...
const useApproveRole = (role : NotApprovedRole | Role) => {
  ...
  const { mutate: approveRole, isPending: isApproving, ...rest } =
...
const useAddRole = (role : Role | NotApprovedRole) => {
  ...
  const { mutate: addRole, isPending: isAdding, ...rest } =
...
const useRemoveRole = (role : Role | NotApprovedRole) => {
  ...
  const { mutate: removeRole, isPending: isRemoving, ...rest } =
...
export const useRoleUtils = (role : Role | NotApprovedRole, guild : Guild) => {
  ...
  const [member] = api.user.memberMe.useSuspenseQuery({ guildId: role.guildId });
...
export const useCreateRole = (guildId: string, name : string | null, onSuccess?: () => void) => {
  const utils = api.useUtils();
