...
rolesRouter.use('create-menu', async ({drizzle, guildUser, msg, getGuild}) => {
  ...
  if (guild.roleMenuId && guild.roleMenuChannelId) {
    const message = await msg.guild.channels
      ...
      .then(channel => channel?.isTextBased() && guild.roleMenuId ? channel?.messages?.fetch(guild.roleMenuId) : null);
