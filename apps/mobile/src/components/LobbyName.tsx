import { generateLobbyName, lobbyChangeSchema , lobbyNameSchema, userIdToPusherChannel } from "@ei/lobby";
...
function LobbyName({ lobby }: Props) {
  ...
  const emojiMap = useMemo(() => emojisByCategory.flatMap((category) => category.data.map((emoji) => emoji)), []);

  const [name, setName] = useState(nameInfo?.name);

  const onNameChange = (newName: string) => {
    pusher?.send_event('client-change-name', {
      name: newName,
  ...
  <View className="bg-primary-900 mb-3 rounded-full flex flex-row p-2 items-center">
    <Pressable onPress={() => setEmojiOpen(true)} className="bg-primary-800 rounded-full w-16 h-16 flex justify-center items-center">
    ...
    <EmojiPicker 
      ...
      onEmojiSelected={(e) => {
        const newName = generateLobbyName(lobby.channel.type, lobby.user, `${e.emoji} ${nameInfo?.name}`, false)?.full;
      ...
      selectedEmojis={nameInfo?.icon ? emojiMap.filter(e => e.emoji === nameInfo.icon).map(e => e.name) : []}
    />
  </View>
  )
}
